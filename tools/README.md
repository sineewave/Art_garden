...
- **提示词通道**（手写 TF-IDF，权重 0.62）——prompt 是人类欲望的直接记录，
  按它分组，"同一种花"的意思就变成"同一种欲望"
- **外观通道**（16×16 灰度指纹 + 27 格色彩直方图，权重 0.38）——保证同种看着也像
...
用 240 条合成语料（5 个真实分组）验证过，聚类纯度 **100%**，自动标签分别是
`robot ruined standing` / `runner night reflections` / `windowsill sitting sunlight`
/ `film soft grain` / `god misty stones`——正好对上 mecha / cyber / cat / portrait / forest。
